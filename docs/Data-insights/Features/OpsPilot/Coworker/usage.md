...
The **Activity** tab shows what Coworker has done over a selected time range. Use the **Plan period** selector in the top right to switch between plan years, and the **Last 30 days / Last 90 days / Full period** toggle to adjust the window within that period. A **suggestions** indicator (for example, **2 suggestions**) in the top right opens the [Optimization Suggestions](#optimization-suggestions) panel.

![!Screenshot](../../../../Coworker/usage-activity.png)

Four summary metrics are shown at the top:

| Metric | Description |
|---|---|
| **Investigations** | Number of investigations Coworker completed in the selected period |
| **Held off** | Investigations Coworker deferred rather than run, split into **allowance** (held back by your spend limits) and **paced** (held back by pacing rules such as cooldowns and repeat-alert limits) |
...
|---|---|
| **Task** | The task name |
| **Kind** | The task type (Heartbeat, Scheduled, Monitoring, or Event source) |
| **Runs** | Number of executions in the selected period |
| **New findings** | Number of new findings generated by that task |

Tasks that have since been deleted still appear (shown as *Deleted task* with no kind) so their past activity stays counted for the period.
...
![!Screenshot](../../../../Coworker/usage-tokens.png)

When Coworker spots a high-impact way to save tokens, it surfaces the headline suggestion as a banner at the top of the tab - for example, *"54% of your token usage this period went on Heartbeat investigations"* - with a one-click action such as **Review signals**. Open the full list from the suggestions indicator (lightbulb) - see [Optimization Suggestions](#optimization-suggestions).
...
- **Coworker**: AI Tokens used by Coworker's task and investigation work
- **Other**: everything else, such as direct chat conversations

Click **Configure** to open **Settings > [Budget & cost](Settings/budget-cost.md)**, where you set how much Coworker can spend - your **Monthly spend cap**, whether **On-demand usage** is allowed past your plan allowance, and the **Warning threshold**. See [Budget & cost](Settings/budget-cost.md) for the full details.

### AI Token usage over time

A bar chart showing AI Token consumption per day across the selected period. Use the **Last 30 days / Last 90 days / Full period** toggle to adjust the window. The chart legend identifies spend by task and includes a **Background work** category for AI Token usage from Coworker's continuous monitoring activity (situation re-checks, tidying, and debriefs) that runs outside of named tasks. When there are many tasks, the smaller ones are grouped together as **Other tasks (N)**.
...
A task with a pending optimization suggestion shows a suggestion badge (a lightbulb with a count) next to its name.
...
A **Total** row at the bottom sums the period total and projected monthly cost across all background activities.

### Optimization Suggestions

Click the **suggestions** indicator (the lightbulb showing a count, for example **2 suggestions**) to open the **Optimisation suggestions** panel - token-saving changes Coworker recommends, to review and apply the ones you want. Each suggestion shows a title, a plain-language explanation of the proposed change, and an estimated monthly AI Token saving (e.g. *Est. save 45 tokens/mo*). Click the chevron to expand the full detail, which includes:
...
To apply every suggestion at once, click **Apply all** at the top of the panel.
