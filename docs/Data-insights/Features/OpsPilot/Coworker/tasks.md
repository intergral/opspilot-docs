...
Create a monitoring task with **Watch** on any insight. You get progressive updates as the task runs, and once its window is up the task winds down on its own. Use monitoring tasks when you want focused attention on something for a while rather than forever.
