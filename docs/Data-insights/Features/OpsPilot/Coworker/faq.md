...
When you first open Coworker, a guided setup conversation walks you through setting your preferences and creating your first task. The quickest way to get value is to enable **OpsPilot Alerts** — this connects Coworker to your existing alert rules so it automatically investigates whenever one fires.
...
Each Coworker is personal to the user it belongs to. However, team members can view **All Team Insights** from the dashboard to see insights across the whole organisation.

### Can I restart the setup?

Yes — click the settings icon on the dashboard and select **Restart setup** to go through the onboarding conversation again.
...
Start with one or two scheduled tasks covering your most critical services, and enable OpsPilot Alerts. Add more tasks over time as you identify gaps. Too many tasks running frequently can increase token costs.
...
Coworker uses OpsPilot AI tokens to run tasks and generate insights. The cost depends on how many tasks you have, how frequently they run, and the model tier selected (Thorough uses more tokens than Efficient). See **Cost & Optimisation** on the dashboard for a full breakdown and projected monthly spend.

### How do I reduce Coworker token usage?

- Review the **optimisation suggestions** in the Cost & Optimisation page — these appear automatically after a task has run a few times and Coworker detects ways it could be improved
